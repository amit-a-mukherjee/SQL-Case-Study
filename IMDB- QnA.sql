...
SELECT year, COUNT(title) AS num_of_movies
FROM movie
GROUP BY year
ORDER BY num_of_movies DESC;

SELECT MONTHNAME(date_published) as month_name,
...
SELECT genre, COUNT(title) AS num_of_movies
FROM genre g
JOIN movie m 
ON g.movie_id=m.id
GROUP BY genre
...
SELECT * FROM ct_avg WHERE movie_rank<=10;
...
SELECT
country, sum(total_votes) AS vote_count
FROM movie m
INNER JOIN ratings r
ON m.id= r.movie_id
...
SUM(CASE WHEN known_for_movies IS NULL THEN 1 ELSE 0 END) AS kfm_nulls
FROM names;
...
ROW_NUMBER() OVER(ORDER BY COUNT(id) DESC) prod_comp_rank
FROM movie m
INNER JOIN
ratings r
ON m.id=r.movie_id
